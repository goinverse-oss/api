...
class CategoriesTest extends ApiTestCase
{
    ...
    {
        $models = factory(Category::class, 2)->create();
        // This category should not be returned in the results
        $this->model();
    ...
    {
        $builder = factory(Category::class);

        if($create) {
            $category = $builder->create();
            $category->contributors()->saveMany(factory(Contributor::class, 2)->create());
            return $category;
        } else {
            return $builder->make();
        }
